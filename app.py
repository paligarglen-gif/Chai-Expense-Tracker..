...
# CSV File Path
CSV_FILE = "chai_transactions.csv"
...
st.markdown(
    "<p style='color: #9C5570; font-family: Quicksand, sans-serif; font-style:"
    " italic; font-size: 1.1rem;'>Keep your boutique, affiliate income &"
    " personal finances organized safely. ✨</p>",
    unsafe_allow_html=True,
)
...
with st.sidebar.form("expense_form", clear_on_submit=True):
  ...
  if trans_type == "Sales (Income)":
    ...
    st.markdown(
        f"<p style='color: #B54565; font-family: Quicksand, sans-serif;"
        f" font-weight: bold;'>Calculated Tubo (Profit): ₱{tubo:,.2f}"
        " ✨</p>",
        unsafe_allow_html=True,
    )
  ...
  if submitted:
    ...
    new_id = int(df_existing["ID"].max() + 1) if not df_existing.empty and not pd.isna(df_existing["ID"].max()) else 1
    ...
    st.sidebar.success("Added and saved safely! 🩰")
    st.rerun()
...
if not df.empty:
  # Convert Date to datetime to extract Month/Year easily
  ...
  personal_expenses = df[
      df["Type"].isin(["Personal Expense", "Loan"])
  ]["Amount"].sum()
  ...
  for month_yr, group in df.groupby("Month-Year"):
    if pd.isna(month_yr):
      continue
    m_income = group[
        group["Type"].isin(["Sales (Income)", "Affiliate Income"])
    ]["Amount"].sum()
    m_savings = group[group["Type"] == "Savings Deposit"]["Amount"].sum()
    m_expenses = group[
        group["Type"].isin(["Personal Expense", "Business Expense", "Loan"])
    ]["Amount"].sum()
...
with st.expander("✨ Open Calculator for Quick Math"):
  ...
  with calc_col1:
    num1 = st.number_input("First Value (₱)", value=0.0, format="%.2f")
  with calc_col2:
  ...
  with calc_col3:
    num2 = st.number_input("Second Value (₱)", value=0.0, format="%.2f")
  ...
  st.markdown(
      f"<p style='color: #B54565; font-family: Quicksand, sans-serif;"
      f" font-weight: bold; font-size: 1.2rem; text-align: center;'>Result:"
      f" ₱{calc_result:,.2f} ✨</p>",
      unsafe_allow_html=True,
  )
...
if not df.empty:
  ...
  with col_save_btn:
    if st.button("Save Changes / Delete 💾"):
      # Clean up helper columns before saving back to CSV
      cleaned_df = edited_df.drop(columns=["Datetime", "Month-Year"], errors="ignore")
  ...
  st.info(
      "Your ledger is currently empty. Add your first record using the sidebar"
      " on the left! 🌸"
  )
